...
# ===== FocusSense imports (NEW) =====
from feature_utils import extract_focussense_features
...
# ===== FocusSense smoothing =====
SMOOTH_ALPHA = 0.8
smoothed_score = None

import requests

def send_to_dashboard(label, score):
    try:
        requests.post(
...
# ===== FocusSense recording label (NEW) =====
FOCUS_LABEL = None  # "focused" or "distracted"
...
# ===== FocusSense initialization (NEW) =====
focussense = FocusSenseInference(
...
# ===== FocusSense sequence buffer (NEW) =====
from sequence_buffer import SequenceBuffer
record_buffer = SequenceBuffer(seq_len=30)   # for saving .npy
infer_buffer  = SequenceBuffer(seq_len=30)   # for live prediction
...
try:
    ...
    while True:
        ...
        if results.multi_face_landmarks:
            ...
            # ===== FocusSense feature extraction (NEW) =====
            face_landmarks = results.multi_face_landmarks[0]
            focussense_features = extract_focussense_features(
                face_landmarks,
                img_w,
                img_h
            )
            print("Feature shape:", focussense_features.shape)

            # ===== DATA RECORDING =====
            if IS_RECORDING:
                print("Recording ON | buffer length:", len(record_buffer.buffer))
                print("Feature shape:", focussense_features.shape)

                if len(record_buffer.buffer) == 0:
                    record_buffer.add(focussense_features)

                else:
                    prev = record_buffer.buffer[-1]
                    diff = np.mean(np.abs(prev - focussense_features))
                    if diff > 0.002:
                        record_buffer.add(focussense_features)
            ...
            if focussense.model is not None:
                prediction, score = focussense.update(focussense_features)


            # ===== FIX D: SCORE SMOOTHING (SAFE) =====
            if score is not None:
                ...
                if smoothed_score is None:
                    ...
                    smoothed_score = int(
                        SMOOTH_ALPHA * smoothed_score +
                        (1 - SMOOTH_ALPHA) * score
                    )
                score = smoothed_score

            if prediction is not None and score is not None:
                send_to_dashboard(prediction, score)    
            
            # ===== DEBUG: inference buffer status =====
            cv.putText(
                frame,
                f"Infer buffer: {len(focussense.buffer.buffer)}/30",
                (30, 240),
        ...
        # Writing the on screen data on the frame
            if SHOW_ON_SCREEN_DATA:
                ...
                # ===== FocusSense on-screen display (OPTIONAL UI FIX) =====
                if prediction is not None and score is not None:

                    # 🔍 UI clarity fix (probability explanation)
                    ...
                    color = (0, 255, 0) if prediction == "Focused" else (0, 0, 255)
                    cv.putText(
                        frame,
                        f"FocusSense: {prediction} ({display_score}%)",
                        (30, 210),
                        cv.FONT_HERSHEY_DUPLEX,
                        0.8,
                        color,
        ...
        # ===== FocusSense data recording (FIXED - NON BLOCKING) =====
        # ✅ FINAL FocusSense recording logic – DO NOT MODIFY

        if key == ord('f') and not IS_RECORDING:
            FOCUS_LABEL = "focused"
            print("Label locked: FOCUSED")
            
        if key == ord('d') and not IS_RECORDING:
            FOCUS_LABEL = "distracted"
            print("Label locked: DISTRACTED")


        if key == ord('r'):
            print("R pressed | IS_RECORDING:", IS_RECORDING, "| LABEL:", FOCUS_LABEL)

            if not IS_RECORDING:
                if FOCUS_LABEL is None:
                    print("⚠️ Press 'f' or 'd' first")
                else:
                    IS_RECORDING = True
                    record_buffer.reset()
                    print(f"▶ Recording started ({FOCUS_LABEL})")
            else:
                IS_RECORDING = False
                seq = record_buffer.get_sequence()
                record_buffer.reset()
                if seq is not None:
                    os.makedirs("data", exist_ok=True)
                    filename = f"data/{FOCUS_LABEL}_{int(time.time())}.npy"
                    np.save(filename, seq)
                    print(f"💾 Saved: {filename}")
                    
                else:
                    print("⚠️ Sequence too short, not saved")
